...
%\title{Modelling the 3-dimensional Dyck language with a 2-multiple Context-Free Grammar}
\title{Towards a 2-Multiple Context-Free Grammar for the 3-dimensional Dyck Language}
...
\institute{
  ...
  \email{\{k.kogkalidis,o.melkonian\}@students.uu.nl}
}

\maketitle
\begin{abstract}
We discuss the open problem of parsing the Dyck language of 3 symbols, $D^3$, using a 2-Multiple Context-Free Grammar. We
attempt to tackle this problem by implementing a number of novel meta-grammatical techniques and
present the associated software packages we developed.

\keywords{Dyck Language $\cdot$ Multiple context-free grammars (MCFG) $\cdot$ Young Tableaux $\cdot$ Spider Webs $\cdot$ Meta-grammars}
\end{abstract}
...
We use $D^3$ to refer to the Dyck language over the lexicographically ordered alphabet $a < b < c$, which generalizes well-bracketed parentheses over three symbols. Denoting with $\#x(w)$ the number of occurrences of symbol $x$ within word $w$, a word belongs in $D^3$ if and only if it s\texttt{a}tisfies the following conditions:
\begin{itemize}
\item[(D1)] $ \#a(w) = \#b(w) = \#c(w) $
...
$MIX$ has already been proven expressible by a 2-MCFG~\cite{salvati}; the class of multiple context-free grammars that operate on pairs of strings~\cite{mcfg}.
...
On the other hand, the prefix condition of $D^3$ allows for partial word movement, while still respecting certain linear order constraints, as observed in natural languages.

Supported by the fact that the language of well-bracketed parentheses, $D^2$, is a simple CFL (i.e. 1-MCFL)
and given that $MIX$ itself is a 2-MCFL, it is reasonable to examine whether $D^3$ can also be modelled by a 2-MCFG.
Such an endeavour proved quite challenging, necessitating careful study of correspondences with other mathematical constructs.
...
A bijection also links Young Tableaux with Spider Webs. More specifically, the act of promotion is isomorphic to a combinatorial \texttt{a}ction on spider webs, namely \textit{web rotation}~\cite{petersen}.
...
\section{Abstract Grammar Specification}\label{sec2}
...
To address the issue of rule size, we employ the notion of \textit{meta-grammars}, loosely inspired by Van Wijngaarden's work~\cite{vanwijn}, which allows a more abstract view of the grammar as a whole.
Meta-grammars have found wide use in describing linguistic phenomena involving discontinuity and word permutation,
giving rise to abstraction techniques that alleviate the need to make all orderings explicit,
such as partially ordered multi-set CFGs~\cite{pomcfg}, phrase structure grammars~\cite{phrase1,phrase2} and
the domain union operation~\cite{domun}.

Specifically, we define $\mathcal{O}$ as the \textit{meta-rule} which, given a rule format, a set of partial orders (over the tuple indices of its premises and/or newly added terminal symbols), and the MCFG dimensionality, automatically generates all the order-respecting permutations. An example of how we can abstract away from explicitly enumerating the entirety of our initial rules is showcased in Fig.~\ref{fig:meta-ins}.
...
The addition of this rule gets us closer to completeness, but we are still not quite there. We have thus far only used a single non-terminal, not utilizing the expressivity that an MCFG allows. To that end, we propose new non-terminals to represent incomplete word \textit{states}; that is, words that either have an extra symbol or miss one. The former are \textit{positive} states, whereas the latter are \textit{negative}. The inclusion of these extra states would allow for more intricate interactions, in line with the CFG that describes the 2-letter $MIX$ language~\cite{lewis1997elements}.

Interestingly, there is a direct correspondence between these non-terminals and the nodes of Petersen's growth algorithm~\cite{petersen}. Fig.~\ref{fig:rules} depicts the growth rules in the exact same web form as proposed by Petersen, modulo node branding.
Briefly, these describe a way of constructing minimal parses of $D^3$ words, as follows:
\begin{enumerate}
\item each character in the original string gets translated into a positive state
\item each growth rule combines adjacent elements on the top, either produ\texttt{c}ing one/two new outputs on the bottom or
cancelling them out
\item the growth process terminates (i.e. the word is parsed) when there remain no dangling strands
\end{enumerate}

A subset of these web-reduction rules are, in fact, precisely modelled by the meta-grammar $\mathcal{G}_2$ presented in Fig.~\ref{fig:g2}. In section~\ref{sec4}, we briefly explain our inability to model the whole set of rules with a 2-MCFG, which would render our grammar complete.
...
\begin{algorithm}
\caption{\textsc{ARIS}: Automatic Rule Inference System}\label{euclid}
\begin{algorithmic}
...
Although $\mathcal{G}_2$ comes close to realizing the growth algorithm, not all of the growth rules presented in 
Figure~\ref{fig:rules} can be translated into a 2-MCFG setting.
In particular, there is no straight-forward way to render the growth rules which produce two edges (i.e. the six bottom-left rules); two neighbouring edges can in turn interact with their next neighbours in a recursive fashion, which can not be accommodated by our current tuple representation.
We cannot, however, rule out the possibility of the growth rules being scalably translatable into m-MCFG rules
without resorting to more expressive formalisms (e.g. context-sensitive rewriting systems).
In fact, such a translation would be a guarantee of completeness.

\paragraph{\textbf{Insights from promotion}}
An interesting question is whether promotion can be handled by a 2-MCFG. If so, it could be worth looking into the properties of orbits, to test for instance if there are promotions within an orbit that can be easier to solve than others. Solving a single promotion and transducing the solution to all equivalent words could then be a guideline towards completeness.
...
We would like to thank Michael Moortgat for introducing us to the problem, providing
insightful feedback and motivating us throughout the process, as well as Jurriaan Hage for
suggesting the use of multi-dimensional Dyck languages in static program analysis.
...
Chatterjee, K., Choudhary, B., Pavlogiannis, A.: Optimal Dyck reachability for
  ...
  Languages  2(POPL), ~30 (2017)

\bibitem{phrase1}
Gazdar, G.: Phrase structure grammar. In: The nature of syntactic
  representation, pp. 131--186. Springer (1982)

\bibitem{phrase2}
Gazdar, G., Klein, E., Pullum, G.K., Sag, I.A.: Generalized phrase structure
...
Kuperberg, G.: Spiders for rank 2 Lie algebras. Communications in mathematical
  physics  180(1),  109--151 (1996)

\bibitem{lewis1997elements}
...
\bibitem{pomcfg}
Nederhof, M.J., Shieber, S., Satta, G.: Partially ordered multiset context-free
  grammars and ID/LP parsing. Association for Computational Linguistics (2003)

\bibitem{petersen}
Petersen, T.K., Pylyavskyy, P., Rhoades, B.: Promotion and cyclic sieving via
  webs. Journal of Algebraic Combinatorics  30(1),  19--41 (2009)

\bibitem{domun}
Reape, M.: Getting things in order. Discontinuous constituency  6,  209--253
  (1996)

\bibitem{reps}
Reps, T.: Program analysis via graph reachability. Information and software
...
Reps, T., Horwitz, S., Sagiv, M.: Precise interprocedural dataflow analysis via
  ...
  on Principles of programming languages. pp. 49--61. ACM (1995)
